...
proc sort data=raw_band; 
	by week lbl_allewgrp_layer3_EW reason8; 
...
run;
data vac_table3_r8;
	set populations_reason8_summary(where=(reason8=1));
run;
data vac_table3_r8;
	set vac_table3_r8(drop=Table F_reason8 reason8 _SkipLine);
	rename 
		Frequency=r8_Frequency
		WgtFreq=r8_WgtFreq
		StdDev=r8_StdDev
		Percent=r8_Percent
		StdErr=r8_StdErr;
...
proc surveyfreq data=raw_band(where=(shot=0)) varmethod=BRR(FAY);
	...
	repweights PWEIGHT1-PWEIGHT80;
	table reason8;
	ods output oneway=populations_reason8_ew;
	by week lbl_allewgrp_layer3_EW;
run;
data vac_table3_r8_ew1;
	set populations_reason8_ew(where=(reason8=1 and lbl_allewgrp_layer3_EW="Working From Home"));
run;
data vac_table3_r8_ew1;
	set vac_table3_r8_ew1(drop = lbl_allewgrp_layer3_EW Table F_reason8 reason8 _SkipLine);
	rename
		Frequency=wfh_r8_Frequency
		WgtFreq=wfh_r8_WgtFreq
		StdDev=wfh_r8_StdDev
		Percent=wfh_r8_Percent
		StdErr=wfh_r8_StdErr;
run;
data vac_table3_r8_ew2;
	set populations_reason8_ew(where=(reason8=1 and lbl_allewgrp_layer3_EW="Essential Worker"));
run;
data vac_table3_r8_ew2;
	set vac_table3_r8_ew2(drop = lbl_allewgrp_layer3_EW Table F_reason8 reason8 _SkipLine);
	rename
		Frequency=ew_r8_Frequency
		WgtFreq=ew_r8_WgtFreq
		StdDev=ew_r8_StdDev
		Percent=ew_r8_Percent
		StdErr=ew_r8_StdErr;
run;
data vac_table3_r8_ew5;
	set populations_reason8_ew(where=(reason8=1 and lbl_allewgrp_layer3_EW="Non-Essential Worker"));
run;
data vac_table3_r8_ew5;
	set vac_table3_r8_ew5(drop = lbl_allewgrp_layer3_EW Table F_reason8 reason8 _SkipLine);
	rename
		Frequency=no_ew_r8_Frequency
		WgtFreq=no_ew_r8_WgtFreq
		StdDev=no_ew_r8_StdDev
		Percent=no_ew_r8_Percent
		StdErr=no_ew_r8_StdErr;
run;
data vac_table3;
	merge 
		vac_table3
...
data vac_table3;
	retain
		...
		wfh_r8_StdErr;
	set vac_table3;
	rr_ewnew_s_pop_perc1 = ew_r8_Percent/no_ew_r8_Percent; *Compare EWs to Non-Ews;
	rr_ewwfh_s_pop_perc1 = ew_r8_Percent/wfh_r8_Percent; *Compare EWs to workers working at home;
	rr_ewtime_s_pop_perc1 = r8_Percent/ref; *Compare within category;
	drop ref;
run;

* Get RR & ConfInt;	
proc sort data=raw_band out=raw_band;
	by week descending reason8 descending allewgrp_layer3_EW;
run;
proc surveyfreq data=raw_band(where=(allewgrp_layer3_EW in (1,2) and shot=0)) varmethod=BRR(FAY) order=data;
	weight PWEIGHT;
	repweights PWEIGHT1-PWEIGHT80;
	table allewgrp_layer3_EW*reason8 /or;
...
* Get RR & ConfInt;
proc sort data=raw_band out=raw_band;
	by week descending reason8 allewgrp_layer3_EW;
run;
...
proc export 
	data = complete_vac_table3 
	outfile="C:\Users\npr8\OneDrive - USNH\2022---Household-PULSE-Data-Analysis\SAS\populations_vac_table3_r8"
	dbms = xlsx
	replace; 
run;

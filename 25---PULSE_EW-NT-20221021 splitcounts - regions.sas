...
proc sort data=week_all out=week_all;
	by week region female hisprace educ allewgrp_layer3_EW disability descending shot;
run;
...
proc surveyfreq data=week_all(where=(week=&wk.)) varmethod=BRR(FAY);
	...
	by week region female hisprace educ allewgrp_layer3_EW disability;
run;
...
Proc export data=PULSE_pop_shot 
	outfile='C:\Users\npr8\OneDrive - USNH\2022---Household-PULSE-Data-Analysis\SAS\PULSE_pop_shot_region.csv' 
	dbms=csv 
	replace;
run;

...
proc sort data=raw_band; 
	by week lbl_allewgrp_layer3_EW reason7; 
...
run;
data vac_table3_r7;
	set populations_reason7_summary(where=(reason7=1));
run;
data vac_table3_r7;
	set vac_table3_r7(drop=Table F_reason7 reason7 _SkipLine);
	rename 
		Frequency=r7_Frequency
		WgtFreq=r7_WgtFreq
		StdDev=r7_StdDev
		Percent=r7_Percent
		StdErr=r7_StdErr;
...
proc surveyfreq data=raw_band(where=(shot=0)) varmethod=BRR(FAY);
	...
	repweights PWEIGHT1-PWEIGHT80;
	table reason7;
	ods output oneway=populations_reason7_ew;
	by week lbl_allewgrp_layer3_EW;
run;
data vac_table3_r7_ew1;
	set populations_reason7_ew(where=(reason7=1 and lbl_allewgrp_layer3_EW="Working From Home"));
run;
data vac_table3_r7_ew1;
	set vac_table3_r7_ew1(drop = lbl_allewgrp_layer3_EW Table F_reason7 reason7 _SkipLine);
	rename
		Frequency=wfh_r7_Frequency
		WgtFreq=wfh_r7_WgtFreq
		StdDev=wfh_r7_StdDev
		Percent=wfh_r7_Percent
		StdErr=wfh_r7_StdErr;
run;
data vac_table3_r7_ew2;
	set populations_reason7_ew(where=(reason7=1 and lbl_allewgrp_layer3_EW="Essential Worker"));
run;
data vac_table3_r7_ew2;
	set vac_table3_r7_ew2(drop = lbl_allewgrp_layer3_EW Table F_reason7 reason7 _SkipLine);
	rename
		Frequency=ew_r7_Frequency
		WgtFreq=ew_r7_WgtFreq
		StdDev=ew_r7_StdDev
		Percent=ew_r7_Percent
		StdErr=ew_r7_StdErr;
run;
data vac_table3_r7_ew5;
	set populations_reason7_ew(where=(reason7=1 and lbl_allewgrp_layer3_EW="Non-Essential Worker"));
run;
data vac_table3_r7_ew5;
	set vac_table3_r7_ew5(drop = lbl_allewgrp_layer3_EW Table F_reason7 reason7 _SkipLine);
	rename
		Frequency=no_ew_r7_Frequency
		WgtFreq=no_ew_r7_WgtFreq
		StdDev=no_ew_r7_StdDev
		Percent=no_ew_r7_Percent
		StdErr=no_ew_r7_StdErr;
run;
data vac_table3;
	merge 
		vac_table3
...
data vac_table3;
	retain
		...
		wfh_r7_StdErr;
	set vac_table3;
	rr_ewnew_s_pop_perc1 = ew_r7_Percent/no_ew_r7_Percent; *Compare EWs to Non-Ews;
	rr_ewwfh_s_pop_perc1 = ew_r7_Percent/wfh_r7_Percent; *Compare EWs to workers working at home;
	rr_ewtime_s_pop_perc1 = r7_Percent/ref; *Compare within category;
	drop ref;
run;

* Get RR & ConfInt;	
proc sort data=raw_band out=raw_band;
	by week descending reason7 descending allewgrp_layer3_EW;
run;
proc surveyfreq data=raw_band(where=(allewgrp_layer3_EW in (1,2) and shot=0)) varmethod=BRR(FAY) order=data;
	weight PWEIGHT;
	repweights PWEIGHT1-PWEIGHT80;
	table allewgrp_layer3_EW*reason7 /or;
...
* Get RR & ConfInt;
proc sort data=raw_band out=raw_band;
	by week descending reason7 allewgrp_layer3_EW;
run;
...
proc export 
	data = complete_vac_table3 
	outfile="C:\Users\npr8\OneDrive - USNH\2022---Household-PULSE-Data-Analysis\SAS\populations_vac_table3_r7"
	dbms = xlsx
	replace; 
run;

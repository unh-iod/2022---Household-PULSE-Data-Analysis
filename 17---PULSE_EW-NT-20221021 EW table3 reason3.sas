...
proc sort data=raw_band; 
	by week lbl_allewgrp_layer3_EW reason3; 
...
run;
data vac_table3_r3;
	set populations_reason3_summary(where=(reason3=1));
run;
data vac_table3_r3;
	set vac_table3_r3(drop=Table F_reason3 reason3 _SkipLine);
	rename 
		Frequency=r3_Frequency
		WgtFreq=r3_WgtFreq
		StdDev=r3_StdDev
		Percent=r3_Percent
		StdErr=r3_StdErr;
...
proc surveyfreq data=raw_band(where=(shot=0)) varmethod=BRR(FAY);
	...
	repweights PWEIGHT1-PWEIGHT80;
	table reason3;
	ods output oneway=populations_reason3_ew;
	by week lbl_allewgrp_layer3_EW;
run;
data vac_table3_r3_ew1;
	set populations_reason3_ew(where=(reason3=1 and lbl_allewgrp_layer3_EW="Working From Home"));
run;
data vac_table3_r3_ew1;
	set vac_table3_r3_ew1(drop = lbl_allewgrp_layer3_EW Table F_reason3 reason3 _SkipLine);
	rename
		Frequency=wfh_r3_Frequency
		WgtFreq=wfh_r3_WgtFreq
		StdDev=wfh_r3_StdDev
		Percent=wfh_r3_Percent
		StdErr=wfh_r3_StdErr;
run;
data vac_table3_r3_ew2;
	set populations_reason3_ew(where=(reason3=1 and lbl_allewgrp_layer3_EW="Essential Worker"));
run;
data vac_table3_r3_ew2;
	set vac_table3_r3_ew2(drop = lbl_allewgrp_layer3_EW Table F_reason3 reason3 _SkipLine);
	rename
		Frequency=ew_r3_Frequency
		WgtFreq=ew_r3_WgtFreq
		StdDev=ew_r3_StdDev
		Percent=ew_r3_Percent
		StdErr=ew_r3_StdErr;
run;
data vac_table3_r3_ew5;
	set populations_reason3_ew(where=(reason3=1 and lbl_allewgrp_layer3_EW="Non-Essential Worker"));
run;
data vac_table3_r3_ew5;
	set vac_table3_r3_ew5(drop = lbl_allewgrp_layer3_EW Table F_reason3 reason3 _SkipLine);
	rename
		Frequency=no_ew_r3_Frequency
		WgtFreq=no_ew_r3_WgtFreq
		StdDev=no_ew_r3_StdDev
		Percent=no_ew_r3_Percent
		StdErr=no_ew_r3_StdErr;
run;
data vac_table3;
	merge 
		vac_table3
...
data vac_table3;
	retain
		...
		wfh_r3_StdErr;
	set vac_table3;
	rr_ewnew_s_pop_perc1 = ew_r3_Percent/no_ew_r3_Percent; *Compare EWs to Non-Ews;
	rr_ewwfh_s_pop_perc1 = ew_r3_Percent/wfh_r3_Percent; *Compare EWs to workers working at home;
	rr_ewtime_s_pop_perc1 = r3_Percent/ref; *Compare within category;
	drop ref;
run;

* Get RR & ConfInt;	
proc sort data=raw_band out=raw_band;
	by week descending reason3 descending allewgrp_layer3_EW;
run;
proc surveyfreq data=raw_band(where=(allewgrp_layer3_EW in (1,2) and shot=0)) varmethod=BRR(FAY) order=data;
	weight PWEIGHT;
	repweights PWEIGHT1-PWEIGHT80;
	table allewgrp_layer3_EW*reason3 /or;
...
* Get RR & ConfInt;
proc sort data=raw_band out=raw_band;
	by week descending reason3 allewgrp_layer3_EW;
run;
...
proc export 
	data = complete_vac_table3 
	outfile="C:\Users\npr8\OneDrive - USNH\2022---Household-PULSE-Data-Analysis\SAS\populations_vac_table3_r3"
	dbms = xlsx
	replace; 
run;

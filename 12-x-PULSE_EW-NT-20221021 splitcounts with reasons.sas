...
proc sort data=week_all out=week_all;
	by week female agegroup1 hisprace educ allewgrp_layer3 WHYNOT descending shot;

proc surveyfreq data=week_all varmethod=BRR(FAY);
	weight PWEIGHT;
	repweights PWEIGHT1-PWEIGHT80;
	table shot;
	ods output oneway=PULSE_pop;
	by week female agegroup1 hisprace educ allewgrp_layer3 WHYNOT;
run;

Proc export data=PULSE_pop 
	outfile='C:\Users\npr8\OneDrive - USNH\2022---Household-PULSE-Data-Analysis\SAS\PULSE_pop_WHYNOT.csv' 
	dbms=csv 
	replace;
run;
